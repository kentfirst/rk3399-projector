...
void TouchInputMapper::cookPointerData() {
	...
	char rotation_status[PROPERTY_VALUE_MAX] = {0};

	property_get("persist.sys.hwrotation.status", rotation_status, "");
	ALOGE("yangbin test 111 rotation_status : %s %d \n", rotation_status, mSurfaceOrientation); 

	#if 1//KentYu 20201009: screen rotate for write board
	if (strcmp(rotation_status, "1") == 0) {
		mSurfaceOrientation = DISPLAY_ORIENTATION_0;
	} else  if (strcmp(rotation_status, "2") == 0){
		mSurfaceOrientation = DISPLAY_ORIENTATION_0;
	} else  if (strcmp(rotation_status, "3") == 0){
		mSurfaceOrientation = DISPLAY_ORIENTATION_180;
	} else  if (strcmp(rotation_status, "4") == 0){
		mSurfaceOrientation = DISPLAY_ORIENTATION_180;
	}
	#else
	if (strcmp(rotation_status, "1") == 0) {
